...
antlrcpp::Any TypeCheckVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
    ...
    if (ctx->expr(0)) {
      if (ctx->expr(1)) {
        // left_expr = M[i,j]
        // Comprueba si no es un tipo error ni un tipo matrix, entonces lanza error
        ...
        TypesMgr::TypeId t4 = Types.createErrorTy();
        bool isLValue = false;
        
        if (Types.isMatrixTy(t1)) {
            t4 = Types.getMatrixElemType(t1);
            isLValue = true;
        }
...
antlrcpp::Any TypeCheckVisitor::visitMatrixAccess(AslParser::MatrixAccessContext *ctx) {
    ...
    // Coge el tipo de la expresion
    TypesMgr::TypeId tCol = getTypeDecor(ctx->expr(1));
    
    // Comprueba si no es un tipo error ni un tipo entero
    if (not Types.isErrorTy(tCol) and not Types.isIntegerTy(tCol)) {
        Errors.nonIntegerIndexInMatrixAccess(ctx->expr(1));
    }
