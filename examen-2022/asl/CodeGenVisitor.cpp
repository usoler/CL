...
antlrcpp::Any CodeGenVisitor::visitAssignStmt(AslParser::AssignStmtContext *ctx) {
  ...
  if (ctx->left_expr()->expr(0)) {
    tLeft = getTypeDecor(ctx->left_expr()->ident());
  }
  
  bool exprIsArrayTy = (offs2 != "");

  if ((Types.isArrayTy(tLeft) or Types.isMatrixTy(tLeft)) and not exprIsArrayTy) {
    if (offs1 != "") {
      ...
      if (Types.isArrayTy(tLeft)) {
        ...
        // Matrix
        int numOfRows = Types.getMatrixRows(tLeft);
        int numOfCols = Types.getMatrixCols(tLeft);
        
        for (int i=0; i<numOfRows; ++i) {
          for (int j=0; j<numOfCols; ++j) {
            std::string rowIndex = std::to_string(i);
            std::string colIndex = std::to_string(j);
            
            std::string tempRowOffs = "%"+codeCounters.newTEMP();
            std::string tempColOffs = "%"+codeCounters.newTEMP();
            std::string tempMultOffs = "%"+codeCounters.newTEMP();
            std::string tempOffs = "%"+codeCounters.newTEMP();
            std::string tempRight = "%"+codeCounters.newTEMP();
            
            code = code 
                  || instruction::ILOAD(tempRowOffs, rowIndex)
                  || instruction::ILOAD(tempColOffs, colIndex)
                  || instruction::MUL(tempMultOffs, tempRowOffs, std::to_string(numOfRows))
                  || instruction::ADD(tempOffs, tempMultOffs, tempColOffs)
                  || instruction::LOADX(tempRight, addr2, tempOffs)
                  || instruction::XLOAD(addr1, tempOffs, tempRight);
          }
        }
      }
    }
  } else if ((not Types.isArrayTy(tLeft) and not Types.isMatrixTy(tLeft)) and exprIsArrayTy) {
    if (Symbols.isParameterClass(addr2)) { // por referencia
        std::string temp = "%"+codeCounters.newTEMP();
        code = code || instruction::LOAD(temp, addr2);
        addr2 = temp;
  ...
  } else if ((Types.isArrayTy(tLeft) or Types.isMatrixTy(tLeft)) and exprIsArrayTy) {
    if (Symbols.isParameterClass(addr1)) { // por referencia
      std::string temp = "%"+codeCounters.newTEMP();
      code = code || instruction::LOAD(temp, addr1);
      addr1 = temp;
...
antlrcpp::Any CodeGenVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  if (ctx->expr(0)) {
    if (ctx->expr(1)) {
      // matrix
      // Expression 'i'
      CodeAttribs     &&  codAtRow = visit(ctx->expr(0));
      std::string         addrRow = codAtRow.addr;
      instructionList &   codeRow = codAtRow.code;
      
      // Expression 'j'
      CodeAttribs     &&  codAtCol = visit(ctx->expr(1));
      std::string         addrCol = codAtCol.addr;
      instructionList &   codeCol = codAtCol.code;
                
      TypesMgr::TypeId t1 = getTypeDecor(ctx->ident());
      std::string numOfElemByRow = std::to_string(Types.getMatrixRows(t1));
        
      std::string tempNumRows = "%"+codeCounters.newTEMP();
      std::string rowOffs = "%"+codeCounters.newTEMP();
      offs = "%"+codeCounters.newTEMP();
      
      instructionList offsCode = instruction::ILOAD(tempNumRows, numOfElemByRow)
                                || instruction::MUL(rowOffs, addrRow, tempNumRows)
                                || instruction::ADD(offs, rowOffs, addrCol);
                                
      code = code || codeRow || codeCol || offsCode;
      ...
      // array
      CodeAttribs     && codAt2 = visit(ctx->expr(0));
      offs = codAt2.addr;
...
antlrcpp::Any CodeGenVisitor::visitMatrixAccess(AslParser::MatrixAccessContext *ctx) {
  ...
  CodeAttribs     && codAt3 = visit(ctx->expr(1));
  ...
  instructionList offsCode = instruction::ILOAD(tempNumRows, numOfElemByRow)
                            || instruction::MUL(rowOffs, addr2, tempNumRows)
                            || instruction::ADD(offs, rowOffs, addr3);
  
  instructionList && code = code1 || code2 || code3 || offsCode;

  
  CodeAttribs codAts(name, offs, code);
...
antlrcpp::Any CodeGenVisitor::visitFunction_call(AslParser::Function_callContext *ctx) {
  ...
  for (int i=0; i<numOfExpr; ++i) {
    ...
    if (Types.isIntegerTy(tExpr) and Types.isFloatTy(tParam)) {
      tempAddr1 = "%" + codeCounters.newTEMP();
      code = code || instruction::FLOAT(tempAddr1, addr1);
    } else if ((Types.isArrayTy(tParam) or Types.isMatrixTy(tParam)) and Symbols.isLocalVarClass(addr1)) {
      tempAddr1 = "%" + codeCounters.newTEMP();
