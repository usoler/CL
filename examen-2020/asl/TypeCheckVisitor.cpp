...
antlrcpp::Any TypeCheckVisitor::visitFilter(AslParser::FilterContext *ctx) {
  ...
  if (not hasErrors and not Types.isErrorTy(t2)) {
    ...
    if (Types.isArrayTy(t2)) {
      TypesMgr::TypeId tElem = Types.getArrayElemType(t2);
      if (not Types.isBooleanTy(tElem) or (Types.isArrayTy(t1) and Types.getArraySize(t1) != Types.getArraySize(t2))) {
        Errors.incompatibleOperator(ctx->op);
        hasErrors = true;
      }
    }
  ...
  // Function F of type A
  visit(ctx->ident(2));
  TypesMgr::TypeId t3 = getTypeDecor(ctx->ident(2));
  
  if (not hasErrors) {
    if (not Types.isErrorTy(t3) and not Types.isFunctionTy(t3)) {
      Errors.incompatibleOperator(ctx->op);
    }
    
    if (Types.isFunctionTy(t3)) {
      TypesMgr::TypeId tResult = Types.getFuncReturnType(t3);
      TypesMgr::TypeId tParam = Types.getParameterType(t3, 0);
      
      if (not Types.isBooleanTy(tResult) or Types.getNumOfParameters(t3) != 1 or (Types.isArrayTy(t1) and not Types.equalTypes(Types.getArrayElemType(t1), tParam))) {
        Errors.incompatibleOperator(ctx->op);
      }
    }
  }
