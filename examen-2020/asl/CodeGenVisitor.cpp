...
antlrcpp::Any CodeGenVisitor::visitSum(AslParser::SumContext *ctx) {
  ...
  std::string tempSum = "%" + codeCounters.newTEMP();
  code = code || instruction::ILOAD(tempSum, "0");
  
  TypesMgr::TypeId tResult = getTypeDecor(ctx);
  ...
  for (int i=0; i<numOfExpr; ++i) {
    ...
    std::string tempAddr1 = addr1;
    if (Types.isIntegerTy(tExpr) and Types.isFloatTy(tResult)) {
      tempAddr1 = "%" + codeCounters.newTEMP();
      code = code || instruction::FLOAT(tempAddr1, addr1);
    }
    
    if (Types.isFloatTy(tResult)) {
      code = code || instruction::FADD(tempSum, tempSum, tempAddr1);
    } else {
      code = code || instruction::ADD(tempSum, tempSum, tempAddr1);
    }
  }

  CodeAttribs codAts(tempSum, "", code);
...
antlrcpp::Any CodeGenVisitor::visitFilter(AslParser::FilterContext *ctx) {
  ...
  TypesMgr::TypeId tArrayAElem = Types.getArrayElemType(tArrayA);
  ...
  int size = Types.getArraySize(tArrayA);
  ...
  code = code 
        ...
        || instruction::ILOAD(temp, "0");
  
  for (int i=0; i<size; ++i) {
    instructionList functionCode = instructionList();
    functionCode = functionCode || instruction::PUSH(); // make space for function result
    
    TypesMgr::TypeId tParam = Types.getParameterType(tFunc, 0);
    
    // Cogemos el valor de a[i]
    std::string index = std::to_string(i);
    functionCode = functionCode 
                  || instruction::ILOAD(tempOffs, index) 
                  || instruction::LOADX(tempParam, addr1, tempOffs);
    
    // Tratamos el valor a[i]
    if (Types.isIntegerTy(tArrayAElem) and Types.isFloatTy(tParam)) {
      functionCode = functionCode || instruction::FLOAT(tempParam, tempParam);
    } else if (Types.isArrayTy(tParam) and Symbols.isLocalVarClass(addr1)) {
      functionCode = functionCode || instruction::ALOAD(tempParam, tempParam);
    }
    ...
    std::string tempFuncRes = "%"+codeCounters.newTEMP();
    functionCode = functionCode || instruction::POP(tempFuncRes); // pop result and store it in temp
    
    
    code = code || functionCode;
    
    // Assign function result to array B[i]
    code = code
          || instruction::XLOAD(addr2, tempOffs, tempFuncRes);
    
    // Increment by one the number of true booleans
    std::string label = codeCounters.newLabelIF();
    std::string labelEndIf = "endIf" + label;
    
    code = code 
            || instruction::FJUMP(tempFuncRes, labelEndIf)
            || instruction::ADD(temp, temp, tempOne)
            || instruction::LABEL(labelEndIf);
