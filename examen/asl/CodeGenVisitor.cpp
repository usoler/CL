...
antlrcpp::Any CodeGenVisitor::visitMap(AslParser::MapContext *ctx) {
  ...
  // Code for A
  CodeAttribs     && codAts1 = visit(ctx->ident(0));
  std::string          addr1 = codAts1.addr;
  std::string          offs1 = codAts1.offs;
  instructionList &    code1 = codAts1.code;
  
  TypesMgr::TypeId tArrayA = getTypeDecor(ctx->ident(0));
  TypesMgr::TypeId tParamA = Types.getArrayElemType(tArrayA);
  
  // Code for B
  CodeAttribs     && codAts2 = visit(ctx->ident(1));
  std::string          addr2 = codAts2.addr;
  std::string          offs2 = codAts2.offs;
  instructionList &    code2 = codAts2.code;
  
  TypesMgr::TypeId tArrayB = getTypeDecor(ctx->ident(1));
  TypesMgr::TypeId tParamB = Types.getArrayElemType(tArrayB);
  
  // Code for F
  CodeAttribs     && codAts3 = visit(ctx->ident(2));
  std::string          addr3 = codAts3.addr;
  std::string          offs3 = codAts3.offs;
  instructionList &    code3 = codAts3.code;
  
  TypesMgr::TypeId tFunc = getTypeDecor(ctx->ident(2));
  TypesMgr::TypeId tArgument = Types.getParameterType(tFunc, 0);
  TypesMgr::TypeId tReturn = Types.getFuncReturnType(tFunc);
  ...
  int numOfParams = Types.getArraySize(tArrayA);  
  for (int i=0; i<numOfParams; ++i) {
    std::string index = std::to_string(i);
    
    std::string tempAddr1 = addr1;
    if (Types.isIntegerTy(tParamA) and Types.isFloatTy(tArgument)) {
      tempAddr1 = "%" + codeCounters.newTEMP();
      code = code 
            || instruction::FLOAT(tempAddr1, addr1);
    }
    
    code = code
          || instruction::ILOAD(tempOffs, index)
          || instruction::LOADX(tempParam, tempAddr1, tempOffs) // get param from A[i]
          || instruction::PUSH() // make space for function result
          || instruction::PUSH(tempParam)
          || instruction::CALL(funcName)
          || instruction::POP()
          || instruction::POP(temp); // pop result and store it in temp
          
          
    std::string tempTemp = temp;
    if (Types.isIntegerTy(tReturn) and Types.isFloatTy(tParamB)) {
      tempTemp = "%" + codeCounters.newTEMP();
      code = code 
            || instruction::FLOAT(tempTemp, temp);
    }
    
    code = code
          || instruction::XLOAD(addr2, tempOffs, tempTemp); // store result in B[i]
  }
...
antlrcpp::Any CodeGenVisitor::visitPotencia(AslParser::PotenciaContext *ctx) {
  ...
  // Calcular base
  CodeAttribs     && codAts1 = visit(ctx->expr(0));
  std::string          addr1 = codAts1.addr;
  std::string          offs1 = codAts1.offs;
  instructionList &    code1 = codAts1.code;
  
  // Calcular exponente
  CodeAttribs     && codAts2 = visit(ctx->expr(1));
  std::string          addr2 = codAts2.addr;
  std::string          offs2 = codAts2.offs;
  instructionList &    code2 = codAts2.code;
  ...
  std::string tempIsWhileEnd = "%" + codeCounters.newTEMP();
  std::string tempOne = "%" + codeCounters.newTEMP();
  std::string tempFloat = "%" + codeCounters.newTEMP();
  
  code = code
        || instruction::ILOAD(tempIndex, "0")
        || instruction::ILOAD(tempOne, "1");
  ...
  if (Types.isFloatTy(t1)) {
    code = code
          || instruction::FLOAD(temp, "1.0")
          || instruction::LABEL(labelWhile)
          || instruction::LT(tempIsWhileEnd, tempIndex, addr2)
          || instruction::FJUMP(tempIsWhileEnd, labelEndWhile)
          || instruction::FMUL(temp, temp, addr1)
          || instruction::ADD(tempIndex, tempIndex, tempOne)
    ...
    code = code 
          || instruction::ILOAD(temp, "1")
          || instruction::LABEL(labelWhile)
          || instruction::LT(tempIsWhileEnd, tempIndex, addr2)
          || instruction::FJUMP(tempIsWhileEnd, labelEndWhile)
          || instruction::MUL(temp, temp, addr1)
          || instruction::ADD(tempIndex, tempIndex, tempOne)
          || instruction::UJUMP(labelWhile)
          || instruction::LABEL(labelEndWhile)
          || instruction::FLOAT(tempFloat, temp);
  }
  
  CodeAttribs codAts(tempFloat, "", code);
